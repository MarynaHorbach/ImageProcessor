...
BitMap Reader::Read() const {
    ...
    if (input.is_open()) {
        try {
            ...
            int64_t new_size =
                bitmap.bm_header.bm_width * bitmap.bm_header.bm_height * bitmap.bm_header.bm_bit_count / 8;
            ...
            if (bitmap.bm_header.bm_width % 4 == 0) {
                ...
                for (size_t i = 0; i < bitmap.bm_header.bm_height; ++i) {
                    input.read(reinterpret_cast<char *>(bitmap.image_array.data() + width_in_bytes * i),
                               width_in_bytes);
                    input.read(reinterpret_cast<char *>(arr_for_padding.data()), padding_size_bytes);
                }
                bitmap.file_header.bm_size += bitmap.bm_header.bm_height * (width_in_bytes + padding_size_bytes);
            }
        } catch (...) {
            throw FileProblemException(
                "Wrong format of the file. Check that it is 24-bit BMP. Try passing no parameters to see help "
                "information.");
        }
        input.close();
    } else {
        throw FileProblemException(
            "Failed to open the file. Try one more time or try to open another file. Current file: " + file_);
    }
    return bitmap;
}
            ...
            if (bitmap.bm_header.bm_width % 4 == 0) {
                ...
                for (size_t i = 0; i < bitmap.bm_header.bm_height; ++i) {
                    output.write(reinterpret_cast<const char *>(bitmap.image_array.data() + width_in_bytes * i),
                                 width_in_bytes);
                    output.write(reinterpret_cast<const char *>(arr_for_padding.data()), padding_size_bytes);
                }
            }
        } catch (...) {
            throw FileProblemException(
                "Failed to write the result. Try one more time or try to use another file-path Pass no parameters to "
                "see help information.");
        }
        output.close();
    } else {
        throw FileProblemException(
            "Failed to write the result. Try one more time or try to use another path. Current path: " + file_);
    }
}