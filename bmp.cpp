...
void BitMap::Upgrade(const std::vector<std::vector<Pixel>>& pixels) {
    ...
    for (size_t i = 0; i < bm_header.bm_height; ++i) {
        for (size_t j = 0; j < bm_header.bm_width; ++j) {
            image_array[index] = static_cast<uint8_t>(pixels[i][j].b);
            ++index;
            image_array[index] = static_cast<uint8_t>(pixels[i][j].g);
            ++index;
            image_array[index] = static_cast<uint8_t>(pixels[i][j].r);
            ++index;
        }
    }
}
